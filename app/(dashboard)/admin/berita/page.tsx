...
export default function AdminBeritaPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/berita?status=${filter}`);
  ...
  const updateStatus = async (id: string, status: string) => {
    ...
    const res = await Swal.fire({
      title: `${confirmMsg} Berita?`,
      text: `Apakah Anda yakin ingin melakukan aksi ${confirmMsg.toLowerCase()} pada berita ini?`,
      icon: 'question',
    ...
    if (res.isConfirmed) {
      ...
      Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
        text: `Berita telah di-${status === "published" ? "publiikasikan" : status === "rejected" ? "tolak" : "update"}.`,
        timer: 1500,
  ...
  return (
    <div>
      <Topbar title="Admin - Moderasi Berita" role={userRole} />
      <div className="page-content">
        <div className="page-header">
          <div className="page-header-left">
            <h2>Moderasi Berita</h2>
            <p>Review, publikasikan, dan kelola konten Berita</p>
          </div>
                    ...
                    <tr key={item.id}>
                      ...
                      <td>
                        <div className="flex gap-2">
                          <Link href={`/berita/${item.id}`} className="btn btn-sm btn-secondary">Baca</Link>
                          <Link href={`/berita/${item.id}/edit`} className="btn btn-sm btn-info">Edit</Link>
                          <button className="btn btn-sm btn-danger" onClick={() => deleteBerita(item.id)}>Hapus</button>
                          {item.status === "pending" && (
                            <>
