...
export default function AdminKatalogPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch my selections (for admin testing or if admin is also a participant)
      if (myProfile?.id) {
      ...
      Swal.fire("Error", e.message || "Gagal memuat data dari server", "error");
    } finally {
      setLoading(false);
    }
  ...
  const handlePilih = async (targetId: string) => {
    ...
    const { isConfirmed } = await Swal.fire({
      title: "Pilih Peserta?",
      text: "Konfirmasi pilihan peserta ini. Maksimal 3 pilihan.",
      icon: "question",
    ...
    try {
      ...
      Swal.fire("Berhasil", "Peserta berhasil dipilih.", "success");
      fetchData();
    } catch (err: any) {
    ...
    return (
      <div className="auth-fallback">
        <div className="auth-card-pdkt">
          ...
          <p>Halaman ini hanya dapat diakses oleh Panitia atau Peserta yang sudah terdaftar dalam <b>Daftar Peserta Mandiri</b>.</p>
              ...
              <div key={item.id} className={`pdkt-card-compact gender-${item.jenisKelamin?.toLowerCase()}`}>
                ...
                <div className="card-body-compact">
                  ...
                  <div className="passion-section-compact">
                    ...
                    <div className="passion-item-compact">
                      <Utensils size={12} />
                      <span className="bit-value truncate-compact"><b>Makanan/Minuman:</b> {item.makananMinumanFavorit || "-"}</span>
                    </div>
                  </div>
                  ...
                  <div className="card-actions-compact" style={{ gap: '10px' }}>
                    <Link href={`/admin/katalog/${item.id}`} className="btn-detail-pdkt" style={{ flex: 1 }}>
      ...
      <style jsx>{`
        ...
        @keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }
          ...
          .pdkt-card-compact { 
             ...
             border: 0.5px solid #3b82f633 !important;
          ...
          .card-avatar-compact, .badge-logged-in, .id-compact { display: none !important; }
          
          .name-compact { font-size: 13px !important; font-weight: 800 !important; margin-bottom: 6px !important; color: #0f172a !important; line-height: 1.2 !important; height: auto !important; }
          .loc-compact { justify-content: center !important; font-size: 10px !important; color: #475569 !important; font-weight: 600 !important; margin-top: auto !important; padding-top: 10px !important; border-top: 1px dashed #e2e8f0 !important; width: 100% !important; }
          
          .qr-print-only { display: block !important; margin: 15px 0 !important; }
        ...
        @media (max-width: 640px) {
          .pdkt-grid-compact { grid-template-columns: 1fr; }
          .explorer-toolbar { flex-direction: column; align-items: stretch; gap: 15px; }
          .filter-groups-pdkt { flex-direction: column; align-items: stretch; gap: 10px; }
