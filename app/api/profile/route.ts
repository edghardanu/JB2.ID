...
export async function GET() {
  try {
    ...
    if (!session.generusId) {
      ...
      // Fallback: Ambil data dari tabel users jika tidak punya generusId (khusus pengurus/admin)
    ...
    const mandiriData = session.generusId 
      ? await db.query.mandiri.findFirst({ where: eq(mandiri.generusId, session.generusId) })
      : null;
