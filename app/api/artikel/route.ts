...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Artikel GET error:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!["admin", "pengurus_daerah", "kmm_daerah", "creator", "desa", "kelompok"].includes(session.role)) {
      return NextResponse.json({ error: "Role Anda tidak diizinkan untuk menulis artikel" }, { status: 403 });
    }
    ...
    console.error("Artikel POST error:", error);
