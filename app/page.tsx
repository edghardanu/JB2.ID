...
async function getHighlightMembers(limit = 8) {
  try {
    return await db
      .select({
        id: users.id,
...
export default async function LandingPage({ searchParams }: { searchParams: { q?: string } }) {
  ...
  const highlightMembers = await getHighlightMembers(8);
    ...
    <>
      <style>{`
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .mem-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); gap: 16px; }
        .mem-card {
          background: white; border: 1px solid var(--border); border-radius: 14px;
          padding: 16px; text-align: center; transition: all 0.2s;
        }
        .mem-card:hover { border-color: var(--primary); background: var(--primary-lt); }
        .mem-avatar {
          width: 64px; height: 64px; border-radius: 50%; background: #f1f5f9;
          margin: 0 auto 12px; display: flex; align-items: center; justify-content: center;
          font-size: 22px; font-weight: 800; color: #cbd5e1;
          border: 3px solid white; box-shadow: 0 4px 8px rgba(0,0,0,0.05); overflow: hidden;
        }
        .mem-avatar img { width: 100%; height: 100%; object-fit: cover; }
        .mem-name { font-size: 13px; font-weight: 700; color: var(--navy); margin-bottom: 3px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .mem-role { font-size: 10px; color: var(--primary); font-weight: 800; text-transform: uppercase; letter-spacing: 0.3px; }
        ...
        <div className="wrap">
          <div className="navbar-inner">
            <div className="nav-links">
              ...
              <div className="nav-item">
                ...
                <div className="nav-dropdown">
                  <Link href="/#profile" className="nav-dropdown-link">Profil Anggota</Link>
                  <Link href="/agenda" className="nav-dropdown-link">Kegiatan</Link>
                </div>
              </div>
              <Link href="/login" className="nav-link">Masuk</Link>
              <Link href="/register" className="nav-link">Daftar</Link>
            </div>
            <form action="/" method="GET" className="nav-search">
              ...
              {query && (
                <Link href="/" style={{ fontSize: 18, color: 'var(--gray-lt)', marginLeft: 8 }} title="Bersihkan">×</Link>
              )}
            </form>
      ...
      {/* ═══ PROFILE ANGGOTA SECTION ═══ */}
      <section id="profile" className="sect-wrap" style={{ padding: '60px 0', borderTop: '1px solid var(--border)', background: '#fff' }}>
        <div className="wrap">
          <div className="sect-hd">
            <span className="sect-hd-title">Profile Anggota</span>
            <Link href="/login" className="sect-hd-more">Gabung Komunitas →</Link>
          </div>
          {highlightMembers.length > 0 ? (
            <div className="mem-grid">
              {highlightMembers.map((m) => (
            ...
            <div className="empty" style={{ border: 'none' }}>
              <div className="empty-icon">👥</div>
              <p className="empty-text">Data anggota sedang dalam pemeliharaan.</p>
            </div>
          )}
        </div>
      </section>
